...
%======================================================================
\section[Introduction \textnormal{\small(1,014 words)}]{Introduction}
\label{sec:intro}
%======================================================================
...
%======================================================================
\section[Requirements and Design \textnormal{\small(1,036 words)}]{Requirements and Design}
\label{sec:design}
%======================================================================
...
%======================================================================
\section[Implementation \textnormal{\small(1,821 words)}]{Implementation}
\label{sec:impl}
%======================================================================
...
\midrule
\texttt{fetchers/} & 299 & One module per platform (Codeforces, AtCoder, LeetCode);
each exposes the same \texttt{fetch\_user\_submissions} interface, hiding three very
different APIs behind one signature. \\
\texttt{models/} & 1{,}333 & Recommenders (\texttt{content}, \texttt{collaborative},
\texttt{hybrid}), the online skill estimator, the unified schema and the serving
engine. \\
\texttt{scripts/} & 3{,}315 & 21 pipeline stages: acquisition, dataset construction,
...
\texttt{implicit} requires a compiled native extension, the module also carries a
TruncatedSVD fallback behind the same interface, so the pipeline degrades to a pure-Python
path rather than failing outright on a machine without a working build toolchain --- a
small piece of defensive engineering that made the results reproducible on a second
machine without intervention.
...
Codeforces prior. Because the two entry points write the same state, seeding and live
correction compose without special-casing, and the \texttt{atcoder+cf\_5} condition of
Section~\ref{sec:results-xplat} is literally a \texttt{seed()} call followed by five
\texttt{update()} calls. The estimate is clipped to $[0,1]$ so that a pathological history
cannot push it off the difficulty scale.
...
already public. The \texttt{UNIQUE(user\_id, platform)} constraint enforces the one
sensible cardinality --- at most one handle per platform per account --- at the schema level
...
\paragraph{Automated tests.} A \texttt{pytest} suite of $36$ tests covers three areas
...
initially failed, and the failure was informative: with a \emph{constant} step size the
estimate does not converge pointwise to the $q$-quantile at all but performs a random
walk whose stationary \emph{mean} is the quantile. The instantaneous estimate therefore
carries a noise floor proportional to $\eta$ --- about $0.04$ on the $[0,1]$ difficulty
scale at the deployed $\eta=0.02$, against $0.007$ at $\eta=0.001$. This does not affect
the seeded estimates of Section~\ref{sec:results-xplat} (\texttt{seed()} sets the
empirical quantile exactly), but it does mean the purely online estimate should be read
as a running average rather than a point value, and it is a property we would not have
stated without having tried to assert it.

\paragraph{Working habits.} Four practices did the rest.
\emph{Baselines as assertions.} A \texttt{random} model is carried through every
experiment specifically as a lower-bound sanity check. Its scores behave exactly as
theory predicts (\ndcg{}@10 $=0.0012$, and $\approx K/|\mathcal{C}|$ for HitRate@$K$),
which is a continuous check that the ranking, masking and metric code are not silently
broken --- if the harness were mis-wired, random would not sit where arithmetic says it
should. \emph{Leakage checks.} Every model's training solves are masked before ranking,
and the fold-in experiments use item factors fitted without the probe user's held-out
items; the $k=0$ degeneracy ($x=0$) was traced analytically to confirm it was the
method's behaviour and not an indexing error. \emph{Adversarial re-testing of surprises.}
The two counter-intuitive results were each re-examined before being believed: the
content model's chance-level performance was diagnosed by building an opposed scorer
(\texttt{diffmatch}) and confirming it moved the metric five-fold in the predicted
direction, and the cross-platform transfer failure was re-tested under three independent
designs before being accepted. \emph{Regenerability as a regression test.} Because the
...
The suite's limits should be stated plainly. It covers the metric, split and skill
components but not the fetchers (which would need recorded API fixtures), the ALS wrapper
(whose numerical behaviour is the upstream library's) or the web layer (which has no
endpoint tests). Coverage is therefore deliberately concentrated where a silent failure
would be both plausible and invisible, rather than uniform; a system intended for
maintenance by others would need the remaining two-thirds.

%======================================================================
\section[The NLP Cross-Platform Auto-Tagger \textnormal{\small(668 words)}]{The NLP Cross-Platform Auto-Tagger}
\label{sec:tagger}
%======================================================================
...
%======================================================================
\section[Evaluation Methodology \textnormal{\small(1,196 words)}]{Evaluation Methodology}
\label{sec:eval}
%======================================================================
...
cannot distinguish one hit from five. Precision and Recall trade against each other and
are both distorted here by variable test-set sizes: because $|\mathcal{R}|$ is capped at
$10$ but often smaller, Precision@10 is bounded above by $|\mathcal{R}|/10$, and
Recall@10 cannot reach $1$ for a user with $20$ held-out items. MRR captures only the
first hit, which matters for a interface showing a short list but ignores everything
...
$200$ training solves counts exactly as much as one with $5$. This is the right choice
for a system judged on its service to individuals rather than on aggregate traffic, but
it means the headline numbers are not dominated by the heavy users who supply most of the
interactions --- and, since heavy users are the hardest to predict
(Section~\ref{sec:results}), it makes the averages lower than an interaction-weighted
alternative would.
...
Per-user \ndcg{}@10 is strongly non-normal and zero-inflated --- most users receive no hit
at all, so the distribution is a spike at zero with a thin right tail --- which rules out a
$t$-test on the raw scores. We therefore test model differences with the paired Wilcoxon
signed-rank test~\cite{wilcoxon1945}, which compares the \emph{same} users under two
models and so removes the large between-user variance in difficulty-of-prediction that
would otherwise swamp the model effect.

Significance alone would not be informative at this sample size: with $n=2{,}459$ paired
observations, differences far too small to matter will reach any conventional threshold.
We therefore report a rank-biserial effect size alongside every $p$-value, and give
bootstrap ($2{,}000$-resample) $95\%$ confidence intervals on each model's mean so that
the results can be read as intervals rather than points. This combination --- paired
non-parametric test, effect size, and interval estimates against a strong baseline --- is
what Dacrema et al.~\cite{dacrema2019} identify as missing from much of the recommender
literature, and following it is part of why two of this project's headline findings are
negative.
...
%======================================================================
\section[Discussion \textnormal{\small(512 words)}]{Discussion}
...
%======================================================================
\section[Conclusion and Future Work \textnormal{\small(182 words)}]{Conclusion and Future Work}
